...
function temaAtual() {
  return document.getElementById("tema")?.value || "pokemon";
}

function aplicarDecoracoesHistorico() {
  const card = document.querySelector(".history-card");
  if (!card) return;
  const tema = temaAtual();
  const criar = (id, cls, src) => {
    let img = document.getElementById(id);
    if (!img) {
      img = document.createElement("img");
      img.id = id;
      img.className = cls;
      img.alt = "Elemento visual do tema selecionado";
      img.onerror = () => {
        if (!img.dataset.fallbackTried && img.src.endsWith(".png")) {
  ...
  criar("historyDecorLeft", "theme-decor theme-decor-history-left", `./assets/images/history-decor-left-${tema}.png`);
  criar("historyDecorRight", "theme-decor theme-decor-history-right", `./assets/images/history-decor-right-${tema}.png`);
}
...
function renderizarTabela(dados) {
  ...
  const linhas = [...dados].reverse().map((item, index) => {
    const nivelClasse = String(item.nivel || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    return `
    <tr class="nivel-${nivelClasse}">
      <td>${index + 1}</td>
      <td>${item.nivel || "-"}</td>
      <td>${item.tema || "-"}</td>
