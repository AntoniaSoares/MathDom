...
import { criarTabuleiro, selecionarPeca } from "../components/Board.js";
...
function mostrarModalFim({ titulo, mensagem, resumo, tema, onMenu, onHistorico, onNovaPartida }) {
  ...
  overlay.innerHTML = `
    <div class="modal-card">
      ...
      <img class="theme-decor theme-decor-result" src="./assets/images/result-decor-${tema || 'pokemon'}.png" alt="Elemento visual do tema" onerror="if(!this.dataset.fallbackTried){this.dataset.fallbackTried='1';this.src=this.src+'.png'}else{this.style.display='none'}">
      <h3>${titulo}</h3>
      <p>${mensagem}</p>
      ...
      <div class="modal-actions">
        <button id="btnModalNovaPartida">Nova partida</button>
  ...
  document.getElementById("btnModalNovaPartida")?.addEventListener("click", () => {
    overlay.remove();
    onNovaPartida?.();
  });
...
export function iniciarJogo(nivel, tema = "pokemon") {
  ...
  hintEl.innerText = `No computador, arraste as ${meta.quantidade} peças. No celular/tablet, toque em uma peça e depois toque na mesa para conectar valores equivalentes.`;

  const gameDecor = document.createElement("img");
  gameDecor.className = "theme-decor theme-decor-game";
  gameDecor.src = `./assets/images/game-decor-${tema}.png`;
  gameDecor.alt = "Elemento visual do tema selecionado";
  gameDecor.onerror = () => {
    if (!gameDecor.dataset.fallbackTried && gameDecor.src.endsWith(".png")) {
      gameDecor.dataset.fallbackTried = "1";
      gameDecor.src = gameDecor.src + ".png";
      return;
    }
    gameDecor.style.display = "none";
  };
  ...
  const finalizarPartida = async (motivo = "concluido") => {
    ...
    mostrarModalFim({
      ...
      tema,
      onNovaPartida: () => iniciarJogo(nivel, tema),
  ...
  operacoes.forEach((op) => piecesPool.appendChild(criarPeca(op, { onSelect: selecionarPeca })));
  ...
  game.append(gameDecor, playerNameEl, statusPanel, hintEl, feedbackEl, boardArea, piecesArea, actions);
}
